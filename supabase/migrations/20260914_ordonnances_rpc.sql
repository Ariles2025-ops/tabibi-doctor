...
-- =====================================================================
-- ETAT : APPLIQUEE le 14/09/2026 (MCP), apres relecture integrale des 488 lignes
--        par le stratege. Deux paires d'yeux, comme le regime d'audit.
--
-- VERIFICATION RELEVEE APRES APPLICATION :
--   plpgsql_check : ✓ 0 defaut sur les CINQ fonctions.
--   search_path   : `public, pg_temp` fige sur les cinq.
--   retours       : create -> uuid (nu, comme le front l'attend) ; les quatre
--                   autres -> void.
--   droits        : les 4 RPC  anon=false / authenticated=true
--                   _presc_valider_medicaments  anon=false / authenticated=FALSE
--                     — le validateur est ferme AUX DEUX : il n'est appelable
--                     que par les trois RPC qui l'utilisent, jamais depuis le web.
--
-- ⚠️  CE QUI N'EST TOUJOURS PAS PROUVE : le comportement a l'execution. La
-- section VERIFICATION plus bas exige une fixture, et `public.prescriptions`
-- est VIDE. « plpgsql_check a 0 defaut » n'est pas « ca marche » — c'est la
-- lecon du `42702` de admin_audit_rebut_list, que la creation n'avait pas vu.
-- Ces cinq fonctions sont POSEES et RELUES ; elles ne sont pas EPROUVEES.
--
-- ⚠️  ET LE DRAPEAU `prescriptions: false` RESTE FERME. Deux verrous avant de
-- l'ouvrir, dans cet ordre :
--   1. la RLS qui s'annule (voir §0.1 du design, et le lot `ordonnances-rls`) ;
--   2. les deux edge functions absentes (voir §0.2) — sans elles, aucune
--      ordonnance ne peut passer a `signed`.
--
