...
| **P1** | ⬜ | Backend | 🔴 **Ordonnances : 4 RPC absentes de la PROD** — `create_prescription_draft`, `update_prescription_draft`, `request_prescription_signature` (`medecin-ordonnance.html`) et `mark_prescription_delivered` (`patient-ordonnances.html`). Vérifié le 29/07 contre `pg_proc` : elles n'existent pas. **Aucun feature flag ne les protège** → échec silencieux (PGRST202 → erreur générique). **À trancher : créer les 4 fonctions, OU neutraliser les 2 pages derrière `TABIBI_FEATURES.prescriptions = false`** comme pour la visio et les avis. Détail : `docs/RPC_INVENTORY.md` | M |
