...
| **P0** | ✅ | Mobile | ~~Scripts internes packagés dans l'APK public~~ — **CORRIGÉ le 29/07** (`mobile/apk-clean`) : `scripts/build-mobile.sh` réécrit en liste blanche (27 pages grand public, 8 dossiers) + garde-fous qui font ÉCHOUER le build si `seo/`, `scripts/`, `api/` ou une page pro/admin réapparaît. ⚠️ **L'APK en ligne reste l'ancien** — la correction ne prend effet qu'au prochain build (lot D) | S |
| **P1** | ✅ | Desktop | Push + PR Phase 3 — **FAIT** (PR #16 + #17 mergées dans `main`) | — |
| **P1** | 🔧 | Backend | 🟡 **Ordonnances : 4 RPC absentes de la PROD — pages neutralisées, implémentation à faire.** *Palliatif livré le 29/07 (branche `fix/ordonnances-flag`)* : flag `TABIBI_FEATURES.prescriptions = false`, boutons masqués + bandeau « bientôt disponible » côté médecin, traçage de délivrance rendu no-op côté patient (téléchargement et partage restent fonctionnels) — plus aucun PGRST202. **CHANTIER SÉPARÉ RESTANT** : écrire et déployer les 4 RPC — `create_prescription_draft`, `update_prescription_draft`, `request_prescription_signature` (`medecin-ordonnance.html`) et `mark_prescription_delivered` (`patient-ordonnances.html`). Vérifié le 29/07 contre `pg_proc` : elles n'existent pas. **Aucun feature flag ne les protège** → échec silencieux (PGRST202 → erreur générique). (`create_prescription_draft`, `update_prescription_draft`, `request_prescription_signature`, `mark_prescription_delivered`) + les versionner, puis repasser le flag à `true`. Détail : `docs/RPC_INVENTORY.md` | M |
| **P1** | 🔧 | Backend | ⚠️ **Schéma prod non versionné** — *tranché le 29/07 (branche `chore/schema-dump`, **SUPPRIMÉE du remote — vérifié le 05/08/2026, `migrations/PROD_SCHEMA_DUMP.sql` jamais produit, chantier à relancer**)* : les 25 RPC existent bien en prod (sauf les 4 ordonnances, ligne ci-dessus), le repo n'en était pas le miroir. Reste à produire `migrations/PROD_SCHEMA_DUMP.sql`. 25 RPC appelées par le front sans définition dans le repo (`admin_*`, `*_prescription_*`, `*_cabinet_*`, `*_api_key*`, `get/upsert_patient_medical_data`) ; aucun `CREATE TABLE patient_medical_data` alors que `patient-profile.html:314-393` lit/écrit dessus | M |
...
| **P1** | 🔧 | Mobile | 🟡 **Lot D — toolchain INSTALLÉE le 06/08/2026, build restant.** JDK Temurin 17.0.20 (`~/Library/Java/JavaVirtualMachines`), cmdline-tools 22, platform-tools 37.0.1, `platforms;android-36`, `build-tools;36.0.0` (`~/Library/Android/sdk`). Installés **sans Homebrew ni sudo** — l'installateur Homebrew exige un mot de passe administrateur non saisissable en session non interactive ; Temurin et les cmdline-tools sont de simples archives. `JAVA_HOME`/`ANDROID_HOME` persistés dans `~/.zprofile`. ⚠️ **`sdkmanager` s'annonce déprécié** au profit d'`android sdk` — à surveiller pour les scripts. | S |
| **P1** | ⬜ | Mobile | 🔴 **Keystore de release en UN SEUL exemplaire — sauvegarde hors machine requise.** `android/tabibi-release.jks` (PKCS12, alias `tabibi`, `PrivateKeyEntry`, SHA-256 fichier `1957fe12…`) est la clé qui signe l'APK distribué. Les deux copies nommées « BACKUP » (`COPIE-2-SECURITE/`, `Tabibi-Backup-2026-07-21/`) portent une **AUTRE clé** (`952590eb…`, mot de passe différent) : elles ne sauvegardent PAS la clé de production. Perte du disque = perte définitive du canal de MAJ pour `dz.tabibi.app`. **Sauvegarder le `.jks` hors machine ET son mot de passe séparément.** | S |
| ~~**P1**~~ | ✅ | Mobile | ~~Chemin contradictoire keystore → signature silencieusement ignorée~~ — **AFFIRMATION FAUSSE, invalidée le 06/08/2026.** `android/app/build.gradle:5` fait `rootProject.file("keystore.properties")` ; `rootProject` vaut `android/` (`android/settings.gradle`), donc il attend `android/keystore.properties` — exactement où le fichier se trouve. Aucune contradiction. La signature n'est pas ignorée : `apksigner verify` sur l'APK réellement distribué (téléchargé depuis le bucket, bit-à-bit identique au build local) donne `Verified using v2 scheme: true`, `SHA-1 07e034833153aede6f3e01ff58e45d7e149bc330`, et `keytool -list -v` sur `android/tabibi-release.jks` renvoie la **même empreinte**. C'était `docs/mobile/DEPLOYMENT_CHECKLIST.md:141` qui indiquait un mauvais chemin — corrigé dans la même PR. | — |
| **P1** | ✅ | Mobile | ~~Bundle id iOS incohérent~~ — **CORRIGÉ le 29/07** : `PRODUCT_BUNDLE_IDENTIFIER` → `dz.tabibi.app` (identité unique actée), 2 clés dupliquées d'`Info.plist` supprimées — les valeurs voulues (`fr-DZ`, portrait-only) n'étaient jamais appliquées, un parseur plist retenant la 1re occurrence. **Non touché volontairement** : le scheme de deep link `com.tabibi.doctor`, partagé avec `AndroidManifest.xml:28` et `js/capacitor-bridge.js:165`. **Reste ouvert** : aucun `DEVELOPMENT_TEAM` → bloquant pour toute soumission App Store | S |
| **P1** | ⬜ | Mobile | 🟠 **Lot C — captcha en WebView mobile (RESTE À FAIRE)** : `js/tabibi-captcha-visible.js:23` ne couvre que `desktop`. Android (`androidScheme:'https'` → origine `https://localhost`, hostname déjà autorisé côté Cloudflare) est *probablement* OK ; **iOS utilise `capacitor://localhost`** — schéma non-http, exactement ce qui a fait échouer Tauri. Correctif probable : `ios: { scheme: 'https' }` + extension du widget visible au mobile. **Validation sur device obligatoire** (dépend du lot D) | M |
| **P1** | ⬜ | Site | `telecharger.html` : aucune détection OS (lien APK unique servi à tous, `:49`), ~~version affichée 1.0.0 vs APK 1.0.1/vc3~~ **corrigé** (`:46` affiche « Version 1.0.1 (build 3) »), ~~bouton « Disponible sur Google Play » mensonger~~ **retiré** (`index.html` ne contient plus aucune occurrence de « Google Play » — vérifié le 06/08 sur `main` ET en production), aucun lien `.dmg`/`.exe` | M |
...
| **P2** | ✅ | Hygiène | ~~`tabibi.apk` 10 Mo tracké~~ — **CORRIGÉ le 29/07** : `git rm --cached` + `.gitignore` (`*.apk`, `*.aab`), distribution par GitHub Releases. L'historique conserve les versions passées (coût déjà payé). Au passage : `telecharger.html` affiche enfin la vraie version (1.0.1 build 3) et le libellé mensonger « Disponible sur Google Play » d'`index.html` est remplacé par « Téléchargement direct / Android (APK) ». ⚠️ Une note du 05/08 affirmait cette mention « CONSERVÉE — l'application arrive » : **c'est faux**, vérifié le 06/08 par `grep -i 'google.\?play'` sur `main` et par `curl` sur la production — zéro occurrence dans les deux cas | S |
