...
- [x] **10.1** Régen des pages SEO — **FAIT le 06/08/2026**. `scripts/generate-seo-pages.mjs` (Node.js, fetch `public_doctors` par batches de 1 000). 490 anciennes pages supprimées → **576 pages** produites (32 890 praticiens, 48 wilayas, 26 spécialités, couples ≥ 10 praticiens). Garde-fou `Disallow: /seo/` levé, `sitemap-seo-local.xml` regénéré (576 URL) et déclaré dans `robots.txt`.
  - ⚠️ **La note précédente de cette ligne était FAUSSE et a failli provoquer une fuite.** Elle affirmait que « SQL_TODO-002 confirme que la vue retourne des noms anonymisés si is_claimed=false → safe pour SEO ». Vérification du 06/08/2026 sur `pg_get_viewdef('public.public_doctors')` : la vue expose `COALESCE(NULLIF(TRIM(dp.full_name),''),'Praticien') AS full_name`, c'est-à-dire **le nom réel, toujours**. Seuls `address`, `latitude` et `longitude` sont masqués pour les fiches non revendiquées. Régénérer « depuis public_doctors » en se fiant à cette note aurait reproduit les 891 patronymes des anciennes pages, puis levé le garde-fou par-dessus. L'anonymisation est faite dans le script, pas dans la vue.
  - Deux affirmations commerciales fausses supprimées au passage : « N praticiens **certifiés** » et « Tous **certifiés Tabibi** » alors que `is_verified` = 0 et `is_claimed` = 0 sur 75 034 fiches ; et un bouton « Prendre RDV » par praticien qui menait à une impasse (aucun de ces praticiens n'est joignable via la plateforme).
  - Les URL sont déclarées **sans `.html`** : Cloudflare Pages sert `/seo/x.html` en 308 vers `/seo/x`, donc canonical, og:url, sitemap et maillage interne pointent tous vers l'adresse réellement servie.
...
## Phase 14 — Doctor onboarding flow (2026-05-23)
- [x] **14.1** `doctor-claim.html` (NEW) — page recherche + claim médecin avec filtre serveur `legacy_id NOT NULL AND is_claimed=false` (14 508 fiches réclamables sur 79 746). Recherche par nom/wilaya/spec via PostgREST, debounce 300 ms, AbortController. Modale claim : saisie N° Ordre (stocké localStorage pour pré-remplissage onboarding) + checkbox honor (loi DZ 18-07 + art. 226-15) + délégation à `tabibiClaim.handle(legacy_id)`. Compteur total dynamique via HEAD count=exact. Lien mailto "Écrivez-nous" pour les 65 238 fiches sans legacy_id.
- [x] **14.2** Mode onboarding `medecin-profile.html?onboarding=1` — banner d'accueil + progress bar 7 champs (bio, tarif, adresse, téléphone, N° Ordre, photo, horaires) + bouton "Sauvegarder et publier ma fiche" qui call saveAll() puis redirect dashboard. Highlight des champs vides obligatoires en jaune. Pré-remplit N° Ordre depuis localStorage (saisi en 14.1). Recompute progress sur input/change avec debounce 250 ms. `tabibi-claim.js` redirige désormais post-claim vers `medecin-profile.html?onboarding=1` au lieu de doctor-dashboard.
- [x] **14.3** CTA hero index.html + footer + badge "Non réclamée" sur cards — CTA dashed "Vous êtes médecin ? Réclamez votre fiche" sous auth-cta-banner (visible tous users). Footer section Médecins : lien priorisé "Réclamer ma fiche" en couleur brand. `docCard()` ajoute badge jaune dashed "Non réclamée" + lien direct vers doctor-claim si `claimed===false`. i18n complet FR/AR/EN pour `cta_doctor_claim`, `foot_claim_fiche`, `badge_unclaimed`, `ob_welcome_title`, `ob_welcome_msg`, `ob_publish_btn`.
...
- Pas de claim self-service au launch (décembre 2026)
